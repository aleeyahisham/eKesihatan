...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        const serviceId = '{{ $appointment->health_service_id }}';
...
<form method="POST" action="{{ route('patient.appointments.update', $appointment) }}">
    ...
    <div>
        ...
        <input id="preferred_date" name="preferred_date" type="date" min="{{ now()->toDateString() }}" max="{{ now()->addDays(30)->toDateString() }}" value="{{ old('preferred_date', $appointment->scheduled_at->format('Y-m-d')) }}" required>
    </div>
    <div>
        <label for="preferred_time">Preferred Time (optional)</label>
        <input id="preferred_time" name="preferred_time" type="time" value="{{ old('preferred_time', $appointment->scheduled_at->format('H:i')) }}">
    </div>
