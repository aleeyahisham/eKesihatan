...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        const doctorOptionsByService = @json($services->mapWithKeys(function ($service) {
        ...
        async function fetchRecommendations() {
            ...
            try {
                const response = await fetch('{{ route('patient.appointments.recommendations') }}?' + params.toString(), {
                    headers: {
...
<form id="book-appointment-form" method="POST" action="{{ route('patient.appointments.store') }}" class="booking-form-shell">
    @csrf
    <div class="booking-layout">
        <div class="booking-column">
            <section class="booking-section">
                ...
                <div class="booking-grid booking-grid--two">
                    <div class="booking-field">
                        ...
                        <select id="health_service_id" name="health_service_id" required>
                            @foreach ($services as $service)
                                <option value="{{ $service->id }}" @selected((int) $selectedServiceId === (int) $service->id)>{{ $service->name }}</option>
                            @endforeach
                        </select>
                        @error('health_service_id')
                            <p class="assistant-error">{{ $message }}</p>
                        @enderror
                    </div>

                    <div class="booking-field booking-field--compact">
                        <label for="preferred_date" data-i18n="Preferred Date">Preferred Date</label>
                        <input id="preferred_date" name="preferred_date" type="date" min="{{ now()->toDateString() }}" max="{{ now()->addDays(30)->toDateString() }}" value="{{ old('preferred_date') }}" required>
                    ...
                    <div class="booking-field booking-field--compact">
                        ...
                        <input id="preferred_time" name="preferred_time" type="time" value="{{ old('preferred_time') }}">
                    </div>
                </div>
            </section>
            ...
            <section class="booking-section">
                ...
                <div class="booking-field">
                    ...
                    <textarea id="notes" name="notes" rows="4" placeholder="Include any symptoms, timing preference, or important context.">{{ old('notes') }}</textarea>
                </div>
            </section>
        ...
        <section class="booking-section assistant-shell" aria-live="polite">
            ...
            <input type="hidden" id="selected_slot_id" name="selected_slot_id" value="{{ old('selected_slot_id') }}">
